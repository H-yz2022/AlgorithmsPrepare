...
- Suppose we multiply x*m:
  x=$\sum_{i=0}^{r-1} x_{b_i} 2^{b_i}$,<br>
  m=$\sum_{i=0}^{r-1} 2^{m_i}$<br>
  Lemme: we can choose the mist<br>
...
- Now we want to pick $m_2^{'}$, <br>$m_2^{'}$ must avoid $m_1^{'}$+$b_j$+$b_z$ (t^2r<r^3 value)
