...
#endif
    //ptep = get_pte(mm->pgdir, addr, 1);   // find/create the PTE for addr
...
/*{
        ...
        {
        if (*ptep & PTE_P) {
            // COW case: page exists but is write-protected for sharing
            struct Page *page = pte2page(*ptep);
            if (page_ref(page) == 1) {
                // no one else references it anymore — safe to just reinstate write access
                *ptep |= PTE_W;
                ...
                page_insert(mm->pgdir, newpage, addr, perm | PTE_W);
            }
        }
